...
//---------------------
{
	...
	if (d < t && n != vec3(0,0,-1))
	{
		...
		{
			hitColor = vec3(0.22, 0.24, 0.26);
			hitType = COAT;
		}
		else //if (n == vec3(-1,0,0)) // right green wall
		{
			hitColor = vec3(0.38, 0.34, 0.29);
			hitType = COAT;
		}
...
//-----------------------------------------------------------------------
{
	...
	boxes[4] = Box( vec3(uPlayerPos.x-uPaddleRadX, uPlayerPos.y-uPaddleRadY, uPlayerPos.z), vec3(uPlayerPos.x+uPaddleRadX, uPlayerPos.y+uPaddleRadY, uPlayerPos.z+3.0), z, vec3(0.08, 0.32, 0.62), COAT);// Player paddle (0.1,0.7,1.0)
	boxes[5] = Box( vec3(uComputerPos.x-uPaddleRadX, uComputerPos.y-uPaddleRadY, uComputerPos.z), vec3(uComputerPos.x+uPaddleRadX, uComputerPos.y+uPaddleRadY, uComputerPos.z+3.0), z, vec3(0.68, 0.16, 0.07), COAT);// Computer A.I. paddle (0.7, 0.1, 1.0)
